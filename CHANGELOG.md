...
删掉 2.30.2 刚加的「提交数守卫」，并把提交数这个字段从两份文档里一并移除。
**无行为改动**（无 `src/` / `ui/` 修改，无需打 tag、无需部署）。

### 为什么：不是实现有 bug，是**选错了守卫字段**
加完当天核算余量才看清：ROADMAP 写 102、HANDOFF 写 103，而实际已 104，
**再正常提交 3 次 CI 就会变红** —— 为一件没人做错的事。

守卫的字段分两类，这个区分是本次最值得记的产出：

| 类别 | 特征 | 例子 |
|---|---|---|
| **可校验的量** | 文档写错才失败 | 版本号（必须等于 `package.json`）、行数（±10% ≈ 900 行余量） |
| **单调漂移的量** | 与文档正确性无关，却每次提交都变 | **提交数** |

给后者加守卫，得到的不是防腐，而是**一个定时闹钟**。而它的修法是「去改一个数字让它
闭嘴」，于是训练出的习惯变成「守卫红了就改文档」而不是「守卫红了说明真有问题」。
...
### Removed — 提交数守卫 + 两处文档字段
根治办法不是调容差，而是认清**这个数字根本不该写进文档**：想知道提交数，
`git rev-list --count HEAD` 一秒就有，写进文档只是凭空造出一个会漂移的副本。

这正是 2.30.2 自己提出的「去重」原则的彻底应用 —— 2.30.2 已用它处理了覆盖率与缺陷计数
（同一数字只留一处），却给提交数加了守卫而不是删掉它。**没有副本就没有漂移，
也就不需要守卫。**

- 删 `test/unit/pure-helpers.test.mjs` 里的提交数守卫，原地留一段注释说明**别再加回来**，
  含上面那个「两类字段」的判据（删掉测试却不留理由，下一个人会重新加一遍）
- 删 ROADMAP 快照与 HANDOFF 现状表里的「提交数」行
- 顺带删掉随之失效的 `existsSync` / `execFileSync` 两个 import（不留死代码）
- 2.30.2 的条目**保留原文**并加一行指回本版本，留住审计链

剩余两条守卫（版本号、测试行数）不动 —— 它们钉的是可校验的量。

共 **359 项**（326 fast + 21 e2e + 12 需凭证 skip），0 失败。

## [2.30.2] - 2026-08-09

修正 ROADMAP 与 HANDOFF 里失真的数字，并把「能静态推出的那些」纳入守卫。
**无行为改动**（无 `src/` / `ui/` 修改，因此无需打 tag、无需部署）。

### 为什么会失真：一份声明自身状态的文档，写完就过时了
`9142` 行、提交 `101`、版本 `2.30.0` 这三个数字**在写下的那一刻全是对的** ——
是 2.30.1 那个 commit 本身把它们推进了一格（新增守卫测试使测试行数变 9299、
提交 +1、版本 +0.0.1）。人工更新追不上这种自指，所以能静态推出的必须交给测试。

### Fixed — ROADMAP 快照的 8 个数字
线上版本（`v2.22.0` → `v2.29.1`，差 7 个版本，会误导接手者去做多余部署）、
测试行数（9142 → 9299）、提交数（99 → 102）、`npm test` 项数（**226 → 360**，差 134，
看着像测试掉了一大片）、all files 覆盖率（78.67% → 76.42%）、dist（70.94% → 66.90%）。

### Fixed — 缺陷计数：`10` 与 HANDOFF 的 `11` 长期矛盾，且 `10` 是错的
拆解写作 `cleanText 7 + WebM 1 + UI 无障碍 1 + CI 守卫 1`，那个 **7 其实是「7 条正则」**
（`docs/research/markdown-cleaning-audit-20260806.md` 审计的是 7 条规则，查出 **5** 个缺陷、
另 2 项判定为 markdown 固有歧义不修）。这个误记恰好掩盖了漏计的一个，于是总数少算 1。
逐版本重数后确认 **11 正确**：`custom_keywords` 空分支遮蔽（2.25.0）1 +
小数被吃与 VS16 emoji 残留（2.26.0）2 + markdown A/B/C/C2/D（2.27.0）5 = `cleanText` **8**，
加 WebM 1 + UI 无障碍 1 + CI 守卫 1。两份文档现在都写 11 并附同一份拆解。

### Changed — 用「去重」处理机器钉不住的数字
覆盖率与缺陷计数无法在不跑工具的前提下校验，而它们漂移的根因是**同一个数字写在多处**
（ROADMAP 曾在三处写 `src/worker.js` 覆盖率：99.47% / 99.51% / 99.2%，自相矛盾）。
现在只在快照表保留一处，命令速查区改为指向它。**没有副本就没有漂移。**

### Added — 三条守卫（`test/unit/pure-helpers.test.mjs`）
既有守卫只钉源码行数、章节与反向链接，**漂移的字段全部在它之外**。补上：

1. HANDOFF 表头声明的版本号必须等于 `package.json` 的版本 —— 直接抓本次的 `2.30.0` vs `2.30.1`
2. 两份文档里的测试行数必须在实际值 ±10% 内（`test/**/*.mjs` 换行符计数）
3. 两份文档里的提交数必须在 `git rev-list --count HEAD` ±5 内

第 3 条在**无 `.git` 或浅克隆**时跳过并说明理由：浅克隆的 `rev-list --count` 给的是错数，
硬断言会让 CI 无故变红 —— 与既有三个文档测试同一考量（它们也刻意不查 git tag）。

变异验证：把版本号、测试行数、提交数分别改错，三条**各自且只各自**变红；
再在一份无 `.git` 的源码副本里跑，确认提交数那条 **skip 并给出原因**，而不是假绿。

> ⚠️ **上面第 3 条（提交数守卫）已在 2.30.3 删除** —— 它当天就被判定为设计错误。
> 本条目保留原文以留审计链，理由见 2.30.3。

共 **360 项**（327 fast + 21 e2e + 12 需凭证 skip），0 失败。

## [2.30.1] - 2026-08-08

新增 **`docs/HANDOFF.md`** —— 给接手者（含其他 AI Agent）的单一入口。**无行为改动。**

### 为什么单开一份文档
现有三份各有职责，但都答不了「接手时最需要知道的事」：
`ROADMAP.md` 记**做什么**、`CHANGELOG.md` 记**做过什么**、`README` 面向**使用者**。
缺的是**为什么这么做**、以及**动手前会踩哪些坑**。后者在本 session 里反复消耗时间，
而它们几乎全都不在代码里 —— 属于最难重建、又最容易丢的那类上下文。

### 内容（10 节）
- **这是什么项目 + 由来**：为什么 `src/worker.js` 是单文件、注释密度异常高、
  `legacy/` 与生产代码毫无关系 —— 因为线上代码原本**没有任何本地副本或版本管理**，
  这个仓库是「反压缩 + 归位 + 补测试 + 修 bug」的产物。
- **版本与部署的当前真相**：`package.json` 2.30.0 而 tag 停在 `v2.29.1`，
  **看着像漏部署其实不是**（2.30.0 只改 CI 与文档，无 `src`/`ui` 改动）。
  但同一模式**踩过一次** —— 三个版本从未打 tag 也就从未部署，含 UI 无障碍修复。
- **贯穿全项目的主线：静默失败**。7 个缺陷列成一张表（1.67 秒截断、WAV 丢 58%、
  Opus 丢 90%、200+0 字节、小数被吃…），据此提炼出核心原则：
  **宁可明确报错，也不要返回一个「看着正常」的错误结果**。
- **刻意不做的 6 件事**及其理由，避免下一个 Agent 当成待办重新提出来。
- **9 个坑**（最省时间的一节）：内联 script 不能有反引号、注释里不能写测试导出的字面名、
  mock 会吞 `console.*`、绝对毫秒断言在 CI 上必红、e2e 的 skip 语义需要防 skip 守卫、
  `| tee` 吞退出码、改正则先想 ReDoS、Workers 并发会串模块级状态、三条平台硬约束。
- **方法论**：五轮审计的真缺陷 : 假警报 ≈ **1 : 4**，且假警报几乎全是同一形态 ——
  **为了验证 X 而手写一个 X 的实现，然后信了它**（列出五个翻车的手写检查器）。
  给出有效判据顺序，以及「荒谬数字就是探针错了」「通过也要看是不是空洞的」两条自检。

### Added — 文档防腐测试
本项目已三次交付过悄悄失真的文档数字，所以这份也有测试盯着：校验四个必备章节、
代码地图行数与实际相符（容差 ±10%）、以及它**被四个主要入口引用**
（一份没人找得到的交接文档等于没写）。变异验证：改错行数 / 从 README 撤链接，各自变红。

### 一处自纠
校验脚本第一版用 `split('\n').length` 数行，比 `wc -l` 多 1，于是四个文件全部报「不符」，
看着像文档写错了。改用换行符个数计。**顺带修正一处真错**：测试代码行数应为 9142
（此前记 9107，是我早先统计后又加了测试）。

共 **345 项**（324 fast + 21 e2e），0 失败。
...
### Fixed — 文档状态过期
- P2 里两项**其实已完成但没划掉**：`cleanText` 脏数据来源注释（2.26.0 做的）、
  版本号漂移（2.28.1 修的 + 加了一致性测试）。
- 「下一步」表里的「观察真实数据」已由后续三轮维度普查回答（voice / format / chunks，
  每轮各抓到一个真 bug），改为已完成并补上「日志驱动的三轮普查」一节。
- **快照全面过期**：写于 08-06，两天后 src 从 1430 → 1708 行、测试从 313 → 357 项、
  提交从 89 → 99。已按实测更新，覆盖率重测为 **99.47% 行 / 97.76% 分支**
  （未覆盖仅 2 处，均为已证不可达的防御分支）。

### Added — 快照防腐测试
本项目已经因「文档数字悄悄失真」踩过**三次**（README 的「90000 字符可达」、
「CPU < 1ms」、「chunk_size=300 约 13500 字符」），而快照本身**没有任何东西在看着它**。
新增测试比对快照声明的行数与实际（容差 ±10%，快照是量级参考不是精确账本）。
只钉代码里查得到的量 —— 覆盖率与提交数需要跑工具或查 git，在浅克隆/无 `.git` 的
tarball 里不可得（与版本一致性测试同一考量）。
变异验证：把行数改回 1430 立刻变红。

### 一处自纠
该测试第一版用 `new RegExp` 拼字符串，转义在 heredoc 里翻倍，匹配不到任何东西 ——
于是它报「快照里必须写明行数」而快照其实写了。改用字面正则。
**「测试失败」先分清是代码错还是断言错**，这条本 session 已复现多次。

### 优先级上调:让 GitLab 也能跑 browser e2e
2026-08-06 GitHub Actions 出现 `major_outage`（官方状态页确认 `Actions: major_outage`），
同一 commit 在 GitLab 通过，而 GitHub 连 runner 都拿不到（job `steps 数: 0`，等 15 分钟被
cancelled）。那天**UI 回归实际上没有任何 CI 防线** —— 21 个 browser e2e 只在 GitHub 跑，
GitLab 因 `node:22-slim` 无 Chrome 全部 skip。这正是「重复跑」的价值，已在 ROADMAP 上调优先级。

共 **344 项**（323 fast + 21 e2e），0 失败。

## [2.29.0] - 2026-08-07

审计最后一个未系统查过的模块:`ui/index.html`(2461 行内嵌 Vue SPA)。已有 19 项 browser
e2e 覆盖主流程,本轮查的是**主流程之外**的东西。报告:`docs/research/ui-audit-20260807.md`。

**1 个真缺陷、4 次假警报** —— 与上一轮全模块审计的比例几乎一样(1:4)。

### Fixed — 6 个表单控件缺正式可访问名称
18 个可见控件里有 6 个既无 `<label>` 关联也无 `aria-label`:

- **语言/地区/性别筛选 `<select>` ×3:连 placeholder 都没有** —— 读屏只念当前值
  「全部语言」,用户无从知道这是什么筛选。
- 音色搜索框、停顿时长输入:仅靠 placeholder,而 placeholder 获得焦点后会消失。
- 输出格式 `<select>`:**有可见 `<label>` 但缺 `for`** —— 与前几轮修滑块 label 同一个问题。

判据刻意**不接受 placeholder 兜底**(规范上它不是可访问名称的正当来源)。修复后 18/18 全过。

### 4 次假警报(本轮最有价值的产出)
- **AudioContext 采样率不一致 → 无害**。两处创建点一个 48000Hz(硬件默认)、一个显式
  24000Hz,且都用 `if (!this.audioContext)` 守卫,谁先跑谁赢 —— 看着像典型变调 bug。
  实测否掉:`AudioBuffer` 自带 `sampleRate`,Web Audio 按 buffer 自己的采样率重采样,
  两种 context 下同一 buffer 的 `duration` **都是 1.0s**。
- **RAF / objectURL 泄漏 → 不存在**。计数不对称(RAF 7 vs cancel 2、create 3 vs revoke 1)
  且 3 处 `running = false` **都不在 finally 里**,理论上异常路径会泄漏。实测 7 轮循环
  (3 成功 + 3 失败 + 基线)后三个指标全稳:RAF 空闲触发 0–1、objectURL 净值恒为 1
  (正是当前可下载的那个)、AudioContext 恒为 1。
- **XSS「1 个注入元素」→ 是页面自己的 `<script>`**。6 载荷 × 4 字段脚本执行 0 次;
  那个"注入元素"在**干净页面**上也存在 —— 我的 `script:not([src])` 命中了 Vue 本体。
- **浅色主题「17/25 对比度不足」→ 探针忽略 alpha**。报 `API Key` 标签 **1.00:1**
  (前景背景完全同色,文字根本看不见)—— 数字本身就荒谬。截图证明浅色主题完全可读。
  原因:往上找背景时取了 `rgba(30,41,59,0.55)` 这个**半透明**覆盖层却忽略 alpha;
  它叠在浅底上实际渲染成 `rgb(124,136,145)`,真实对比度 4.03:1。
- 附带:`voice-item` 的 `tabIndex=[-1,-1,-1,0]` 被我误报成「3 个元素键盘不可达」——
  **那正是 WAI-ARIA roving tabindex 的标准实现**,实测聚焦与方向键均可行。

### 方法论(连续两轮一致的模式)
假警报的共同形态:**为了验证 X,我手写了一个 X 的实现,然后信了它。** 手写 XML 良构检查器、
EBML 扫描器、对比度计算器、注入元素选择器、RAF 泄漏探针 —— 五个都错了。
有效判据顺序:**可观测的最终结果**(截图/资源累积/时长)> **标准工具** >
**被测代码自己的解析器** > 手写;另外「荒谬数字就是探针错了的信号」、
「通过也要看是不是空洞的」。

### 测试
新增 2 项 browser e2e:全控件可访问名称、roving tabindex 正确性。
后者专门**防止把对的实现改坏** —— 「把 tabindex 全改成 0」看着更可访问,
实则让 322 个音色各占一个 Tab 停留点,键盘用户要按几百次才能走出该组。
变异验证:去掉 aria-label → 第一项红;tabindex 全改 0 → 两项红。

共 **343 项**(322 fast + 21 e2e),0 失败。

## [2.28.1] - 2026-08-06

修我在 2.28.0 里弄红 CI 的问题,并把那个**只存在于 CI 的守卫**搬进本地测试。

### Fixed
- 2.28.0 的一条注释里写出了测试专用导出的**字面名字**。CI 有一步 `grep -q` dist 里有没有
  这个名字(确认构建剥掉了测试面),而**构建只剥导出、保留注释** —— 于是两边 CI 都变红,
  而我本地 `npm test` 是绿的。注释已改为不写字面名。
- **根因不是那条注释,是守卫只在 CI 里**:`npm test` 跑不到它,本地绿 ≠ CI 绿。
  已把同一检查加进 `test/regression/fixed-bugs.test.mjs`,本地门禁与 CI 就此对齐。
  用运行时拼接名字(与该文件已有的 `CLOSE_SCRIPT` 同一手法),免得测试文件自己踩自己的 grep。
  变异验证:把注释改回去,新测试立刻变红。

共 340 项(322 fast + 19 e2e)。

## [2.28.0] - 2026-08-06

对 `src/worker.js` 剩余 **7 个模块**逐个做契约式审计(约 250 用例,绝大部分零上游成本)。
报告:`docs/research/module-audit-20260806.md`。

| 模块 | 用例 | 违反 |
|---|---|---|
| `smartChunkText` | 112 | 0 |
| `getSsml` 注入面 | 16 | 0 |
| WebM/EBML 合并 | 20 | **1** |
| WAV 合并 | 8 | 0 |
| 鉴权与路由 | 18 | 0 |
...
**只有 1 个真缺陷,且生产不可达。** 现有规则足以支撑当前内容 —— 这轮主要是确认了这一点。

### Fixed
- `mergeWebmChunks([])` 抛 `Cannot read properties of undefined`。fuzz 12 种畸形字节 +
  8 种组合里唯一会抛的输入。生产不可达(调用点有 `length > 1` 前提),但它是**纯字节函数**
  且被 `__test__` 导出,按项目自己的规则「无法处理时降级留痕、不要抛」就该返回 `null`
  让调用方裸拼接。抛出去会冒到 `getVoice` 的 catch 变成 500,而正确行为是「合并放弃、音频照发」。

### Documented — 参数校验的两处决策此前没写理由
两种策略并存:`speed`/`pitch` 越界**400 拒绝**,`concurrency`/`chunk_size` **clamp 夹紧**。
依据是「用户听得出来吗」:前者直接决定音频听起来什么样(要 4.0 倍速静默得到 2.0,
音频完好、就是不对);后者是**性能旋钮**,实测 `concurrency=1e9` 与 `=20` 的上游调用次数
**完全相同**。另记 `clamp` 语义:能转成有限数就夹到 `[min,max]`,转不成才用 fallback ——
所以 `chunk_size=0` 得到 **50(min) 而非 300(default)**,靠「180 字符切 4 块」实测确认。

### 4 次假警报(本轮最有价值的产出)
250 个用例里**真缺陷 1 个、假警报 4 次、我的预期写错 9 项**。假警报的共同模式:
**我为了验证 X 而手写了一个 X 的实现,然后信了它。**

- `getSsml` 报「2 例 XML 非良构」—— 是我手写的检查器错了(正则处理不了引号属性值里含 `/`
  的 `<break time="500ms"/>`)。改用 Python `xml.etree` 后 16/16 良构。
- WebM 时间戳「非单调」**连续错了三次**:
  1. 全文件扫 `E7 88` —— 扫出 `1.65e19`、`4.9e18` 这种荒谬值夹在 15450 和 15950 之间,
     那本来就是**音频负载里恰好出现的字节**。
  2. 改成结构化扫描 —— 匹配到 **0 个**,于是「✅ 单调」是**空数组上的空洞通过**,
     比错误结论更危险,因为它看着是通过。
  3. 用生产解析器但字段名猜成 `timecode`(实际 `tc`)—— 打印一串空值又报非单调。
  第四次才对:**57 clusters 一个不丢、严格递增到 27900ms、总时长与三块之和吻合**。
- 参数校验报「9 项违反」全是**我的预期写错**:我以为非法 `speed` 会静默落回默认,
  而代码是精确 400 拒绝 —— 代码更好,也更符合「绝不静默吞掉错误输入」。

**有效做法**:优先用标准工具(`xml.etree`/`ffprobe`)而非手写检查器;优先用**被测代码自己的
解析器**读它自己的格式;荒谬数字就是探针错了的信号;「通过」也要看是不是空洞的
(0 个元素上的 `every()` 恒为 true)。

### 测试
新增 3 项(WebM 空输入降级、cluster 守恒与时间戳单调、总时长吻合),共 **339 项**
(321 fast + 19 e2e)。变异验证:去掉空输入守卫、破坏时间戳偏移各让对应测试变红
(后者报「第 19 个:9000 -> 0」,精确指到分块边界)。

## [2.27.0] - 2026-08-06

系统审计 `remove_markdown` 的 7 条正则(约 60 用例 + 真实世界文本,零上游成本)。
**查出 5 个真缺陷**,另有 2 项判定为 markdown 固有歧义、明确不修。
报告:`docs/research/markdown-cleaning-audit-20260806.md`。

### 核心认知:`\w` 只是 ASCII,中文不在其中
这是本轮所有修复的基础,也是我**一开始搞错**的地方。我给 `__` 加了 `\w` 边界,却在注释里
写「`**` 刻意不加,因为中文没有词边界,加了会打破中文粗体」。**这是错的** ——
变异测试发现「给 `**` 加边界后测试没变红」,与注释矛盾,查下去:`/\w/.test('重')` 是
**false**,JS 的 `\w` 等价 `[A-Za-z0-9_]`。所以 `\w` 边界对中文强调**零影响**。
认知纠正后,`**` 与单 `*` 都能安全加边界 —— 于是两个原本被我判为「固有歧义」的问题
变成了「可修」。**如果我没去核对那条注释,这两个 bug 会一直留着。**

### Fixed
- **`__` 吃掉标识符里的双下划线**。原来 `/(\*\*|__)(.*?)\1/` 一条管两个符号,于是 `__`
  拿不到 `\w` 保护,而**下面单 `_` 那条早就有了** —— 同一份代码对同类输入两种待遇。
  实测:`变量 my__temp__value` → `变量 mytempvalue`、`MAX__SIZE__LIMIT` → `MAXSIZELIMIT`。
- **URL 含括号漏出 `)`**。`[^)]*` 在第一个 `)` 就停,维基百科链接实测 4/4 全残留:
  `见 [铝](https://zh.wikipedia.org/wiki/铝_(元素))` → `见 铝)`,那个 `)` 会被念成「右括号」。
  改用 `\((?:[^()]|\([^()]*\))*\)` 支持一层配平括号,仍是线性的。
- **算式与标识符里的星号被当强调**:`结果是 2*3*4` → `结果是 234`、`面积 a*b*c` → `面积 abc`、
  `x**2**y` → `x2y`、`file**name**.txt` → `filename.txt`。四条强调规则现在**统一**都有边界。
- **相邻反引号残留**:`` `a``b` `` → `` ab` ``(首尾数量可不同)。改用反向引用要求一致,
  顺带更快(无需回溯探测)。

### 一处只有端到端验证才能发现的问题
给 `**` 加完边界后,`x**2**y` 变成了 `x*2*y` —— 被**下游的单 `*` 规则**接手了。
**修一条正则却被后一条抵消**,单测每条正则都看不出来。所以单 `*` 的边界里还要排除 `*` 本身。
已加专门测试(「the ** and * rules do not undo each other」),变异验证过它会变红。

### 明确不修(markdown 固有歧义,非实现缺陷)
- `__init__ 方法` 与真粗体 `__粗体__` 语法上**无法区分**(两侧都是空白),CommonMark 也判为强调。
  修它要维护「Python 特殊方法名单」之类启发式,误伤风险高于收益。
- `*.txt 和 *.md` 与 `*斜体*` 同构,强修会伤合法斜体。

两项都记进报告,免得后人重复调查。

### ReDoS 复核
本项目曾因 `\[(.*?)\]\(.*?\)` 在 16KB 输入上跑到 **36 秒**(CPU 上限 10ms),所以每次动这些
正则都重测。新引入的正则各自构造攻击:391KB 混合最坏 **4.8ms**,全部随长度线性;
最坏合法请求(50000 字符全选项)中位 **0.51ms**。

### 测试
新增 7 项,共 337 项(319 fast + 19 e2e)。**逐个验证过变红**,含 7 个变异。
其中一个变异(给 `**` 加边界)**存活**了 —— 而它存活恰恰暴露了上面那条错误注释。
**变异存活通常意味着测试有缺口,这次意味着注释在说谎。**

## [2.26.0] - 2026-08-06

顺着 ROADMAP P2 的「`cleanText` 缺脏数据来源注释」去看,结果**查出两个真 bug**,
其中一个是数据损坏级、且 **UI 默认开启**。

### Fixed — remove_citation_numbers 把小数的整数部分吃掉了
- 正则 `/\s\d{1,2}(?=[.。，,;；:：]|$)/` 把 ASCII 句点无条件当作句末标点,于是**分不清
  句号与小数点**:" 3.14159" 里的 " 3" 后面紧跟 ".",被当成「编号+句号」删掉,只剩 ".14159"。
  实测 **7/7 个含小数的句子被破坏**:

  | 输入 | 旧输出 | 用户听到 |
  |---|---|---|
  | `圆周率 3.14159` | `圆周率.14159` | 「点一四一五九」 |
  | `价格 12.50 元` | `价格.50 元` | 「点五零元」 |
  | `温度 36.5 度` | `温度.5 度` | 「点五度」 |

- **影响面**:`removeCitation` 在 UI 里是**默认开启**的(`form.cleaning.removeCitation = true`),
  所有含小数的网页请求都受影响。响应是 200 + 合法音频 + 数字是错的 —— 与 WAV/Opus 截断
  同一类静默失败。
- 修法:ASCII 句点后**不能紧跟数字**(那是小数);逗号同理(`1,234` 是千分位)。
  全角 `。` 不受影响(中文不用它做小数点)。实测引用编号 6/6 仍正确删除、小数千分位 9/9 完好。

### Fixed — remove_emoji 漏掉「靠 VS16 变成 emoji」的那批
- `\p{Emoji_Presentation}` 只覆盖**默认就以图形呈现**的 emoji。`❤️` `☀️` `⭐️` `☎️` 这些
  默认是文本样式、靠 VS16(U+FE0F) 才变成 emoji,不在该属性里,于是清理后**残留并被念出来**。
  实测「我❤️你」清理后仍是「我❤️你」。
- **刻意不用整个 `\p{Extended_Pictographic}`**:那会连 **© 和 ™ 一起删掉**(实测两者
  `Extended_Pictographic=true`、`Emoji_Presentation=false`)。「©2026」念成「2026」是
  另一种数据损坏 —— **拿一个 bug 换另一个 bug 不算修好**。改为加一条 VS16 限定的规则,
  只命中「明确被标记为要当 emoji 显示」的。实测 emoji 清除 7/7、正常符号保留 7/7
  (© ™ € → ★ ① ℃)。

### Fixed — 一个「记录现状而非契约」的测试
原有测试名为 *"remove_emoji leaves text-presentation symbols alone (**documents current
scope**)"* —— 它断言 `❤️` 会被保留。那不是范围决定,是 bug:调用方要求「不要 emoji」却拿到
一个被念出来的 emoji。**测试名里的「documents current scope」本身就是信号** ——
它记的是代码做了什么,而不是代码该做什么。已改为断言真正的契约。

### Added — P2 原条目:脏数据来源注释
`remove_urls` / `custom_keywords` / `remove_line_breaks` 三处补上「脏数据从哪来、不容错的
后果是什么」,遵循项目自己的铁律(容错代码必须注明脏数据来源)。
其中 `remove_line_breaks` 注明了**必须放在最后**的原因:前面每条清理都会产生新的连续空白。

### 一处注释自纠 + 因此发现的第三个隐患
我最初写「空关键词会让正则匹配任意位置,把文本切碎」—— **查证后发现说法不准**:
`"hello".replace(/a||b/g, "")` 返回 `"hello"`,看着毫无变化。真实危害是**遮蔽**:
空分支在每个位置先匹配成功,后面的分支永远轮不到 —— `"banana"` 得到 `"bnn"`(b 没被删),
而正确的 `/a|b/g` 得到 `"nn"`。因为替换成空串,**肉眼看不出「少删了」**。
已按实测改写注释,并补测试(现有测试只覆盖了「全是分隔符」的 no-op,没覆盖
「真关键词 + 一个空项」—— 而 UI 里一个尾随逗号就能产生后者)。

### 测试
新增 6 项,共 329 项(311 fast + 19 e2e)。逐个验证过变红,含 4 个变异:
回退引用编号正则、去掉 VS16 规则、emoji 改用整个 Extended_Pictographic(误删 © ™)、
去掉空关键词 filter。

## [2.25.0] - 2026-08-06

继续用日志看第三个维度:**chunks**。与音色普查不同,分块行为可以**本地穷举**,所以这轮约
8000 次测量是零上游成本的,只有边界验证打了 4 次生产。
报告:`docs/research/chunk-distribution-20260806.md`。

### Fixed — 413 文案宣称一个自相矛盾的容量
- 旧文案在拒绝一个 **50000 字符** 的请求时,却说「当前 chunk_size 下最多约 **50580** 字符」
  —— 宣称 50580 可行,而 50000 已经被拒。根因:`45 × chunk_size` 是**乘积上界**,只在每块
  填满时成立;而 `MAX_INPUT_CHARS=50000` 是先于分块的硬上限,所以任何 >50000 的数字都不可达。
  **这正是 v2.20.0 在 README 里修掉的「90000 字符不可达」同一类错误 —— 当时改了文档,
  没改错误文案。**
- 新文案给**二分实算**出来的可行值:「这段文本(50000 字符)请把 chunk_size 调到约 **1125**
  以上」。线上实证 1124→413、1125→200,与本地预测**逐字符吻合**。
  成本 1.54ms 中位,只在已经要返回 413 的路径上跑,且不打上游(`upstream=0`)。

### 为什么不用系数估算 —— 试过,不行
`ceil(len/45)` 乘 1.05 / 1.15 / 1.25 / 1.4 / 1.6,对 88 个(字符数 × 形态)组合逐一验证,
**没有一个系数全通过**(分别失败 10/4/3/1/1 例)。因为填充率不是常数而是**量化效应**:
`chunk_size` 略小于句长整数倍时每块浪费近一整句 —— 实测 82 字符句在 `chunk_size=125` 下
填充率仅 **65%**,而无标点文本 100%。
二分的前提「分块数随 chunk_size 单调不增」已验证:4 形态 × 50→2000 逐值扫描
**7800 点 0 违反**;取整到 25 的倍数后 **44/44 仍可行**。

### 实测数据(此前文档没有的)
- **默认 `chunk_size=300` 的真实容量是 12179–13500 字符**(随形态),README 原写「约 13500」
  偏乐观 —— 英文散文只有 12179。
- **跑满 50000 字符需要 `chunk_size` ≥1125**(最坏形态 1150)。**此前文档完全没说该调到多少**,
  只说「调大」—— 这是真正的痛点,现已由新文案 + README 表格解决。
- `MAX_CHUNKS=45` **合理,不建议改**:由平台 50 subrequest 硬限决定,留 5 个给 token 与语音列表。
- 默认 `chunk_size=300` **合理但有取舍**:覆盖多数用例且首字节最快;超限时由 413 引导调参。

### 测试
新增 2 项:闭环验证「拒绝 → 按建议重试 → 200」(12 个形态 × 长度组合)、守护二分前提的单调性。
共 325 项(306 fast + 19 e2e)。

**一处测试设计自纠(变异测试抓到的)**:这两个测试的第一版只用 50000 字符输入,结果
**把「1.05 系数」这个错误实现放过了** —— 那个系数在 **5000 字符**处才失效,短输入才有区分力。
加入 5000/20000 两个长度后变异被抓到。
教训:参数化测试要覆盖参数空间里「机制会改变」的那一段,不只是极值。

## [2.24.1] - 2026-08-06

用 v2.23.0 的日志继续看**全部 322 个音色**,约 420 次真实上游调用。
完整报告:`docs/research/empty-audio-sweep-20260806.md`,探针:`scripts/research/sweep.mjs`。

### 结论
- **没有「坏音色」**:322/322 全部能合成自己语种的文本(延迟中位 230ms、p95 602ms)。
  所以不需要维护音色黑名单 —— 问题只出在**文本与音色的配对**上,那是调用方可控的。
- **空音频的规律是「文本书写系统 ≠ 音色书写系统」**,不是「语种不同」。9×9 交叉矩阵
  62/90 组合为空,而**同书写系统 0 失败**。zh-CN 音色读英文完全正常,可见语种不是判据。
- **两个例外**:拉丁文本对**任何**音色都有输出;**Multilingual 音色对 9 种书写系统全部
  有输出(9/9)**。后者是跨语种场景的正解,已写进双语 README 与 UI 的「仅多语言」提示
  (此前只有筛选开关,没解释它为什么重要)。

### ⚠️ 发现一类现有 502 拦不住的情形
希腊文送非希腊音色**不是空音频,而是错误音频**。同一段希腊文实测:
`el-GR-Athina` 3.26s(正常)、`en-US-Ava` **11.04s**(逐个念字母名,长 3.4 倍)、
`zh-CN-Xiaoxiao` **1.78s**(大部分被跳过)。字节非零,所以 `upstream_empty_audio` 不触发。
**判定:不额外拦** —— 判断「音频内容是否正确」需要时长/文本比之类的启发式,误伤风险高
(正常语速本就因语种和 `speed` 差异很大);且与空音频不同,这种情况用户听一遍就知道,
不是静默失败。出路是文档与 UI 引导,不是服务端猜测。

### 一处过程自纠(这次差点发出错误结论)
第一版普查报告 **30/322 失败**,涉及 bn/gu/kn/ml/si/ta/te、km/lo/my、he —— 看着像重大发现。
**实际是我的探针错了**:我把这些语言按「书写系统组」归类并给整组发同一段文本,
但 Indic 组里只有 hi/mr/ne 用天城文,孟加拉/泰米尔/泰卢固/卡纳达/马拉雅拉姆/僧伽罗/古吉拉特
**各有自己的字母**;东南亚组我发泰文,而 km/lo/my 各自不同;RTL 组我发阿拉伯字母,而 he 是
希伯来字母。
**失败集合与「我发错文字的语言集合」完全重合** —— 这个完美相关性本身就是探针有错的信号。
改成按**语言**给母语文本后,异常从 30 变成 **0**。那 30 个「失败」其实是 30 次
「新 502 正常工作」的确认。
教训:分组前先确认组内成员真的共用同一套字母;失败集合与自己某个实现选择完美相关时,
先怀疑探针。

### 测试
新增 3 项真网 e2e(`test/e2e/real-synth.test.mjs`,需 `EDGETTS_E2E=1`),钉住上述两条
README 宣称,并抽样普查里我一度误判的那几种文字。已用真凭证跑通 9/9,无凭证时干净 skip。
双向确认有区分力:502 被删会让第一项变红;502 若写成「所有跨语种都拒」会让后两项变红。

共 323 项(304 fast + 19 e2e,另 12 项真网 e2e 需凭证)。
