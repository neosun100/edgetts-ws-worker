...
**新上线的日志立刻抓到一个真 bug** —— 这正是做 P1-6 的目的。用 voice/format/chunks 矩阵
(20 个组合)打线上、再从 `wrangler tail` 取日志聚合时发现的。

### Fixed — 上游返回空音频被原样透传成「成功」
- **音色对输入文本的书写系统零覆盖时,上游返回 `200` + `content-length: 0`,并不报错。**
  线上实测:中文 / 日文 / 纯标点送给 `en-US-AvaNeural`,**5/5 次全是 0 字节**;
  而同一音色送英文正常(12240B)、同一段中文送 `en-US-AvaMultilingualNeural` 正常(10656B)。
  `zh-CN` 音色读英文也没问题 —— 所以这不是「语种不匹配」,而是「该音色对这套书写系统零覆盖」。
- 原样透传就是**又一次静默成功**:调用方拿到格式合法的空音频,与真结果无法区分
  (本项目已为 WAV / Opus 各修过一次同类)。UI 侧早已拒绝零字节流,但**直连 API 的调用方
  没有这层保护**。现在非流式返回 502 `upstream_empty_audio`,并指出可执行的出路
  (换对应语种音色 / 用 Multilingual 音色)。
- **流式无法拒绝**(响应头早已发出,状态锁在 200)。改为在日志里记
  `phase: "stream_empty"` + `bytes: 0` —— 这是它唯一能被观测到的地方。诚实交代这个限制,
  而不是假装修好了。

### Added
- 日志新增 `bytes` 字段(响应音频字节数)。**没有它,零字节响应在聚合里根本看不出来** ——
  这也正是我第一版日志差点漏掉这个 bug 的原因:19 项测试全绿、维度齐全,但没有 `bytes`,
  `status:200` 的空响应看着完全正常。

### 一处方法论自纠
上一轮我说「tail 里结构化日志行数: 0」,那是**我的解析器错了**,不是日志没出来 ——
`wrangler tail --format=json` 输出的是**多个 pretty-print JSON 对象首尾相接**,不是 JSONL,
按行解析必然得 0。当时 `grep` 明明找到了行,两个结果矛盾时我应当立刻怀疑解析器。
已改用大括号配平切分,顺带能取到 Workers 运行时的 `cpuTime` / `wallTime` / `outcome`。

### 线上聚合实证(20 组合矩阵)
```
status:  200×15  400×1        format:  mp3×8  pcm×3  wav×2  opus×2
chunks:  1×10    4×5          stream:  false×13  true×2
voice:   zh-CN-Xiaoxiao×12  en-US-Ava×1  en-US-AvaMultilingual×1  zh-CN-liaoning-Xiaobei×1
```
生产 `cpuTime` 实测 **5ms**(含日志),平台预算 10ms。

### 测试
新增 4 项,共 323 项(304 fast + 19 e2e)。逐个验证过变红,含 3 个变异:
去掉零字节拦截、`stream_empty` 无条件发出(会误判正常流)、去掉 `bytes` 字段。

## [2.23.0] - 2026-08-06

落地 ROADMAP P1-6:**每个请求输出恰好一行结构化 JSON 日志**。这是 P1 里最后一项,也是唯一
会碰生产热路径的一项 —— 所以先量了 CPU、再验了并发安全,才写。

### Added — 结构化日志
- 每请求一行 JSON,**成功的请求也有**。之前 18 个日志点里 17 个只在异常路径,唯一那条正常
  路径日志记的是 token 有效期 —— 一个成功的 200 请求不产生任何日志,于是 5xx 率 / p99 /
  重试率**都没有分母**。字段清单见双语 README 的「可观测性」一节。
- console 级别与 status 对应(5xx→`error`、4xx→`warn`、其余 `log`),这样「5xx 率」不必
  解析 JSON 就能先粗筛。
- **绝不记** API key 与 input 文本(连哈希、连截断都不记,那是调用方的内容)。`chars` 只带长度。

### Fixed — 流式日志记的是假数据
- 流式的响应头在合成开始**之前**就发出,所以漏斗里那次 emitLog 会在合成还没跑时执行。
  实测记出 `ms:6, upstream:0`,而该请求实际打了 **4 次上游、耗时 192ms**。
  **这比没有日志更糟**:它会把每个流式请求的 p99 与上游用量都算低,而数字看着完全正常。
- 改为流式自己在流真正结束时记(`phase: "stream_end"`),修复后同一请求记出
  `ms:159, upstream:4`。中途断流记 `phase: "stream_broken"` + 上游状态码 ——
  响应头一发出 HTTP 状态就锁在 200 了,**日志是断流唯一看得见的地方**。

### 两处设计决定(都有实测支撑)
- **每请求一个上下文对象,不用模块级「当前请求」变量。** Workers 单 isolate 并发处理请求、
  在每个 `await` 处交错。**写代码前先验证了那个错误形状**:3 个并发请求(延迟 30/5/15ms)
  用模块级变量记日志,三条全部记成最后开始的那个 —— 这种错在日志里看不出来,数据看着很合理。
  已由测试钉住(改成模块级会让它变红)。
- **日志埋在 `handleRequest` 这一个漏斗上**,不是 `handleSpeechRequest` 的 20 个出口。
  逐个包裹既冗长、又必然漏掉以后新增的那个。错误码通过**只 clone >=400 的响应**取回 ——
  几 MB 的音频体永远不 clone;加 `x-error-code` 响应头那条路被否掉了(会把内部错误码暴露给
  公网,且响应头参与缓存键)。

### CPU 成本(先量再改)
序列化一行实测 **0.00013ms**,占最坏合法请求(约 3.2ms)的 **0.004%** —— 比同机噪声
(0.121ms)低三个数量级,端到端根本测不出来。已加测试盯住这个**比例**(<5%),免得以后加字段
悄悄吃进 10ms 预算。断言比例而非绝对毫秒:绝对阈值在 CI 上不可移植,这条教训本 session 已
犯过两次。

### 测试
新增 15 项(`test/integration/structured-logs.test.mjs`)+ 1 项 CPU 比例测试,共 319 项
(300 fast + 19 e2e)。**逐个验证过对着它所守护的实现会变红**,含 8 个变异:
改模块级共享、去掉成功路径日志、把 input 文本记进去、级别全用 log、不计重试、
回退流式日志、断流记成 stream_end、往日志加未写进文档的字段。

其中**日志 schema 文档测试当场抓到我自己刚写的 prose 缺陷**(section 定位用了 `\n## `,
而我写的是 h3),并能双向抓漂移:日志多字段而文档没写、或文档写了日志没发。
