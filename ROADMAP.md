...
三天里 P0 全清、P1 全部关闭（3 已裁定、5 量化后判定不做、**6 已完成**、7 架构不可行、
4/8 已完成）。**P1 到此结束。**
...
| **3. 观察真实数据再做决定** | 日志刚上线，还没有生产数据 | **下一步应该是「看」而不是「改」**：等几天真实流量后，用 voice/format/chunks 分布回答「322 个音色映射表还值得维护吗」「MAX_CHUNKS=45 与默认 chunk_size=300 合理吗」。这正是做 P1-6 的目的 |
| 4. GitLab 跑 browser e2e | GitLab `node:22-slim` 无 Chrome，19 个 browser e2e **全跳过** | 但 GitHub 已在跑 19 个，重复收益有限 |
| 5. `cleanText` 脏数据来源注释 | 违反项目自己的「容错代码必须注明脏数据来源」铁律 | 纯可维护性，无行为风险 |
| 6. TS 化 / 前端拆分 | `ui/index.html` 已 2460 行 | 大改动、零用户可见收益，**不建议现在动** |
