...
[![Tests](https://img.shields.io/badge/tests-359%20passing-brightgreen?logo=nodedotjs&logoColor=white)](#开发)
[![Coverage](https://img.shields.io/badge/coverage-99.47%25-brightgreen)](#开发)<br/>
...
- `?multilingual=true` —— 只返回多语言音色（322 个里的 12 个）。**文本书写系统不固定时就该用
  这些**：线上普查实测 12 个全部对 9 种书写系统都有输出，而单语音色遇到自身覆盖不到的文字会
  返回空音频（90 个交叉组合里 62 个是空的，详见
  [普查报告](docs/research/empty-audio-sweep-20260806.md)）。
...
首字节略慢，但余量大得多。**413 会直接告诉你该用多大的 `chunk_size`** —— 针对你这段文本
实算出来的，不是估的。

该调多大取决于文本，因为标点会让块提前收尾。实测（见
[分布普查](docs/research/chunk-distribution-20260806.md)）：

| `chunk_size` | 撞 413 前的最大字符数 | 填充率 |
|---|---|---|
| 300（默认） | 12179–13500（随文本形态） | 90–100% |
| 500 | 22279–22500 | — |
| 1125 | 典型散文可跑满 50000（硬上限） | 88–95% |
| 2000（上限） | 50000 且有余量 | 91–100% |

线上实证：50000 字符英文散文在 `chunk_size=1124` 被拒、`1125` 通过 —— 与本地预测**完全一致**。
无标点长句填充更满、需要的更小（某形态 533 即可）；全是短句的文本需要更大。
